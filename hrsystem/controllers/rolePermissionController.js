...
const getAllRolePermissions = async (req, res) => {
    try {
      ...
      const result = await pool.request().execute("GetAllRolePermissions");
  
      if (result.recordset.length === 0) {
        return res.status(404).json({ message: "No se encontraron Permisos de los Roles." });
      }
  
      res.status(200).json(result.recordset);
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: "Error al obtener Permisos de los Roles." });
    }
  };
...
const deactivateRolePermission = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Permiso de rol desactivado exitosamente' });
    } catch (error) {
        res.status(500).json({ message: 'Error al desactivar permiso de rol', error });
    }
};
...
const deleteRolePermission = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al eliminar permiso de rol', error });
    }
};
