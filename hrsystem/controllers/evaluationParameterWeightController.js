...
const createEvaluationParameterWeight = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Peso del parámetro de evaluación agregado exitosamente' });
    } catch (error) {
        res.status(500).json({ message: 'Error al agregar el peso del parámetro de evaluación', error });
    }
};
...
const getAllEvaluationParameterWeight = async (req, res) => {
    try {
        ...
        if (result.recordset.length === 0) {
            return res.status(404).json({ message: 'No se encontraron pesos de parámetros de evaluación' });
        }

        res.status(200).json(result.recordset);
    } catch (error) {
        res.status(500).json({ message: 'Error al obtener los pesos de parámetros de evaluación', error });
    }
};
...
const updateEvaluationParameterWeight = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Peso del parámetro de evaluación actualizado exitosamente' });
    } catch (error) {
        res.status(500).json({ message: 'Error al actualizar el peso del parámetro de evaluación', error });
    }
};
...
const deleteEvaluationParameterWeight = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Peso del parámetro de evaluación eliminado exitosamente' });
    } catch (error) {
        res.status(500).json({ message: 'Error al eliminar el peso del parámetro de evaluación', error });
    }
};
